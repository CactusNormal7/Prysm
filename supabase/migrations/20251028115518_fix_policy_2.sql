-- ================================================================
-- SAFE, IDEMPOTENT SUPABASE MIGRATION SCRIPT
-- ================================================================
...
CREATE POLICY "Users can view accessible rooms"
  ...
  USING (
    ...
    -- or user invited and accepted
    OR id IN (
      SELECT i.room_id
      FROM public.room_invites i
      WHERE i.invitee_id = auth.uid() AND i.status = 'accepted'
    )
    -- or user participates (checked only via room_participants.user_id)
    OR id IN (
      SELECT p.room_id
...
-- ================================================================
-- ROOM PARTICIPANTS POLICIES (no recursion)
-- ================================================================
...
CREATE POLICY "Users can view participants of accessible rooms"
  ...
  FOR SELECT
  USING (
    -- room is public or user is creator/invited
    room_id IN (
      SELECT r.id
      FROM public.rooms r
      WHERE r.type = 'public'
